...
interface FileUploadProps {
  ...
  /** Human-readable override for the accepted-types hint (e.g. "PDF o Word")
   *  — MIME strings like the docx one don't read well auto-derived. */
  ...
  const validateFileSize = useCallback(
    (file: File): FileError | null => {
      if (file.size > maxFileSize) {
        return {
          message: `El archivo supera ${formatBytes(maxFileSize)}`,
          code: "FILE_TOO_LARGE",
        };
      }
      return null;
  ...
  const validateFileType = useCallback(
    (file: File): FileError | null => {
      ...
      if (
        ...
        return {
          message: `Formato no soportado — usa ${acceptedFileTypesLabel ?? acceptedFileTypes.join(", ")}`,
          code: "INVALID_FILE_TYPE",
        };
      }
      return null;
    },
    [acceptedFileTypes, acceptedFileTypesLabel]
  );
  ...
  return (
    ...
    >
      <div className="group relative w-full rounded-xl bg-background p-0.5 ring-1 ring-border">
        <div className="absolute inset-x-0 -top-px h-px w-full bg-gradient-to-r from-transparent via-gold/25 to-transparent" />

        <div className="relative w-full rounded-[10px] bg-muted/40 p-1.5">
          <div
            className={cn(
          ...
          >
            ...
            >
              <div className="absolute inset-x-0 top-0 h-[20%] bg-gradient-to-b from-gold/10 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 h-[20%] bg-gradient-to-t from-gold/10 to-transparent" />
              <div className="absolute inset-y-0 left-0 w-[20%] bg-gradient-to-r from-gold/10 to-transparent" />
              <div className="absolute inset-y-0 right-0 w-[20%] bg-gradient-to-l from-gold/10 to-transparent" />
              <div className="absolute inset-[20%] animate-pulse rounded-lg bg-gold/5 transition-all duration-300" />
            </div>

            <div className="absolute -top-4 -right-4 h-8 w-8 bg-gradient-to-br from-gold/25 to-transparent opacity-0 blur-md transition-opacity duration-500 group-hover:opacity-100" />
              ...
              <AnimatePresence mode="wait">
                {status === "idle" || status === "dragging" ? (
                        ...
                        {acceptedFileTypesLabel ??
                          (acceptedFileTypes?.length
                            ? `${acceptedFileTypes
                                ...
                                .toUpperCase()}`
                            : "SVG, PNG, JPG o GIF")}{" "}
                        {maxFileSize && `— máx. ${formatBytes(maxFileSize)}`}
                      </p>
                    </div>

                    <button
                      className="group flex w-4/5 items-center justify-center gap-2 rounded-lg border border-border bg-background px-4 py-2.5 font-semibold text-foreground text-sm transition-all duration-200 hover:bg-muted"
                  ...
                  >
                    ...
                    <button
                      className="flex w-4/5 items-center justify-center gap-2 rounded-lg border border-border bg-background px-4 py-2.5 font-semibold text-foreground text-sm transition-all duration-200 hover:bg-muted"
                      onClick={resetState}
                      type="button"
                    >
                      Cancelar
            ...
            <AnimatePresence>
              {error && (
                <motion.div
                  ...
                  className="absolute bottom-4 left-1/2 -translate-x-1/2 transform rounded-lg border border-destructive/20 bg-destructive/10 px-4 py-2"
                  exit={{ opacity: 0, y: -10 }}
                  initial={{ opacity: 0, y: 10 }}
                >
