...
import type { BookingArtist, BookingData, BookingLocation } from "@/lib/booking/get-booking-data";
...
/** The wizard's whole state, as a flat string map — the exact shape of a
 *  URL query string, but a shell decides where it actually lives: /book
 *  reads/writes it to searchParams (deep-linkable), the global drawer
 *  keeps it in local state (no navigation). */
export type BookingParams = {
  locationId: string | null;
  serviceId: string | null;
  variantId: string | null;
  preference: "specific" | "any" | null;
...
export const EMPTY_BOOKING_PARAMS: BookingParams = {
  locationId: null,
  serviceId: null,
  variantId: null,
  preference: null,
...
export function BookingWizardContent({
  ...
  setParams: (next: Partial<BookingParams>) => void;
  /** Leaves the flow entirely — going back past the first step, or the
   *  drawer's own close (X/swipe/outside-click/Escape) once its exit
   *  animation finishes. What that means differs per shell: /book
   *  navigates home; the global drawer just closes in place. */
  requestClose: () => void;
}) {
  const {
    locationId,
    ...
    startsAt,
    paymentMethod,
    paymentDetail: paymentDetailRaw,
    rescheduleToken,
  } = params;
  const paymentDetail = paymentDetailRaw ?? "";

  // Local-only: expanding a service-with-variants must never touch params
  // (it's still the same step) — only the final leaf choice (a service with
  // no variants, or a chosen variant) commits and advances the wizard.
  // Committing on every intermediate click used to read as "it took me
  // somewhere else" even though it was still step 2.
  ...
  const [locationStepDone, setLocationStepDone] = useState(() => !!params.locationId);
  ...
  // distinct from the local-only cascade below, which never touches params
    ...
    return (
      <DrawerContent>
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-3 overflow-y-auto px-6 pb-10 text-center">
          <p className="font-sans text-xs tracking-[0.3em] text-gold uppercase">Reserva enviada</p>
  ...
  // Step sequence and the param whose absence gates entry into each one
  ...
  function goBack() {
    ...
    const cleared: Partial<BookingParams> = {};
    for (const param of target.params) cleared[param as keyof BookingParams] = null;
    setParams(cleared);
  }
  ...
  const whenText = startsAt
    ? (() => {
        ...
        return `${capitalize(dt.toFormat("cccc"))} ${dt.toFormat("d")} ${capitalize(dt.toFormat("LLLL"))}, ${dt.toFormat("HH:mm")}`;
      })()
    : "";
  ...
  // The current selection, serialized for the "needs login" handoff below —
  // whichever shell is hosting this (in-place drawer or the /book route
  // itself), logging in is always a real navigation away and back, so it
  // always has to land on the URL-backed /book route to survive that.
        ...
        <Questionnaire item={currentStep} onItemChange={handleItemChange}>
          ...
          <QuestionnaireItem name="ubicacion">
            ...
            <div role="radiogroup" className="flex flex-col gap-2">
              {locations.map((loc: BookingLocation) => (
                <button
                  key={loc.id}
                  type="button"
                  role="radio"
              ...
              <div className="flex flex-col gap-6">
                ...
                <ul className="divide-y divide-border rounded-md border border-border">
                  {services.map((s: Service) => {
                    ...
                    const sVariants = variants.filter((v: Variant) => v.service_id === s.id);
                    return (
          ...
          <QuestionnaireItem name="artista">
            ...
            <div role="radiogroup" className="flex flex-col gap-2">
              ...
              {locationArtists.map((a: BookingArtist) => (
                <button
                ...
                >
                  {a.headshot_url ? (
                    <div className="relative size-14 shrink-0 overflow-hidden rounded-sm">
                      <Image src={a.headshot_url} alt={a.display_name} fill sizes="56px" className="object-cover" />
                    </div>
                  ) : (
          ...
          <QuestionnaireItem name="hora">
            ...
            {loadingSlots && <p className="font-sans text-sm text-muted-foreground">Buscando horarios…</p>}
          ...
          <QuestionnaireItem name="pago">
            ...
            <div className="flex flex-col gap-1">
              ...
              {paymentMethod === "transferencia" && !paymentDetail && (
                <p className="font-sans text-xs text-destructive">Elige Llave Bre-B o Nequi para continuar.</p>
              )}
            </div>
          </QuestionnaireItem>
          ...
          <QuestionnaireItem name="confirmar">
            <div className="flex flex-col gap-6">
              ...
              <div className="rounded-md border border-border p-4 font-sans text-sm">
                <p>
                  <span className="text-muted-foreground">Sede:</span> {locations.find((l) => l.id === locationId)?.name}
                </p>
                <p>
                  <span className="text-muted-foreground">Servicio:</span> {selectedService?.name}
                ...
                {homeService && (
                  <p>
                    <span className="text-muted-foreground">Domicilio:</span> {homeAddress} —{" "}
                    {homeServiceZones.find((z) => z.id === homeZoneId)?.name}
                  </p>
                )}
                <p>
              ...
              {userEmail === undefined ? null : userEmail ? (
                ...
                <div className="flex flex-col gap-3 rounded-md border border-border p-4">
                  <p className="font-sans text-xs tracking-[0.2em] text-muted-foreground uppercase">Cuenta requerida</p>
                  <p className="font-sans text-sm text-foreground">
                    Para reservar necesitas iniciar sesión — así podemos enviarte la confirmación y dejarte reagendar
                    o cancelar desde tu cuenta.
                  </p>
                  <p className="font-sans text-sm text-gold">
                    ✓ Tu selección queda guardada — inicia sesión y vuelves directo aquí.
                  </p>
                  <Link
                    href={`/auth/login?next=${encodeURIComponent(`/salon/${salon.slug}/book?${loginNextParams.toString()}`)}`}
